...
class GameManager {
  ...
  _shareX = () => {
    ...
    const textToTweet = encodeURIComponent(
      `I just scored ${this.#score} in the Rupee's game :) ${window.location.href} `
    )
  ...
  _shareFb = () => {
    SoundManager.play(SOUNDS_CONST.SMALL_CLICK)
    const facebookShareUrl = `https://www.facebook.com/raihanulislamnahid44${window.location.href}`
